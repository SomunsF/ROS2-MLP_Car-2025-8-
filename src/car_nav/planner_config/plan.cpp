  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, plugin_name_ + ".input_size", rclcpp::ParameterValue(364));
  nav2_util::declare_parameter_if_not_declared(
  ...
  node->get_parameter(plugin_name_ + ".input_size", input_size_);
  ...
  RCLCPP_INFO(logger_, "  - input_size: %d", input_size_);
  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  try {
    ...
    torch::Tensor input_tensor = prepareModelInput(*current_scan_, *current_imu_, local_goal);
...
torch::Tensor EndToEndController::prepareModelInput(
  const sensor_msgs::msg::LaserScan & scan,
  const sensor_msgs::msg::Imu & imu,
  const std::vector<float> & local_goal)
{
  std::vector<float> features;
  features.reserve(input_size_);

  for (float range : scan.ranges) {
    if (std::isnan(range) || std::isinf(range)) {
      features.push_back(scan.range_max);
    } else {
      features.push_back(range);
    }
  }
  
  features.push_back(static_cast<float>(imu.angular_velocity.z));
  features.push_back(static_cast<float>(imu.linear_acceleration.x));
  
  features.push_back(local_goal[0]);
  features.push_back(local_goal[1]);
  
  if (static_cast<int>(features.size()) != input_size_) {
    RCLCPP_ERROR(logger_, "FATAL: Input feature size mismatch! Expected %d, but got %zu.",
                input_size_, features.size());
    throw std::runtime_error("Input feature size mismatch for model");
  }
  
  torch::Tensor tensor = torch::from_blob(features.data(), {1, input_size_}, torch::kFloat);
  return tensor.clone().to(torch::kCPU);
}
